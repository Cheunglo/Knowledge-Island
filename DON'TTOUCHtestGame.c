...
void testMakeAction (void) {
   ...
   a.actionCode = START_PUBLICATION;
   makeAction (g, a);
   ...
   a.disciplineFrom = STUDENTS_BQN;
   a.disciplineTo = STUDENTS_BPS;
   makeAction (g, a);
   assert (getStudents (g, UNI_A, STUDENTS_BPS) == 1);
   printf ("...passed!");
   disposeGame (g);
...
void testgetStudents (void) {
   ...
   while (index < 4) {
      ...
      assert (getStudents (g, UNI_A, STUDENT_BPS) == 3);
      assert (getStudents (g, UNI_A, STUDENT_BQN) == 3);in
      assert (getStudents (g, UNI_A, STUDENT_MJ) == indexA2 + 1);
      assert (getStudents (g, UNI_A, STUDENT_MTV) == indexA1 + 1);
